...
def filter_objects_by_property_value(objects_to_filter: list, property_name: str, property_value: str):
    try:
        ...
        raise ValueError(
            f'Unable to determine type for the "{property_name}" property.'
...
def check_for_duplicated_ids(objects_to_check: list):
    ...
    if duplicates := [
        property_value
        for property_value in property_values_counter
        if property_values_counter[property_value] > 1
    ]:
        raise ValueError(
...
def check_for_broken_id_references(builds_objects: list, sources_objects: list, docs_objects: list):
    ...
    refs_to_missing_sources = [
        source_id for source_id in referenced_source_ids if source_id not in source_ids]

    doc_ids = [doc['id'] for doc in docs_objects]
    referenced_doc_ids = [build['docId'] for build in builds_objects]
    refs_to_missing_docs = [
        doc_id for doc_id in referenced_doc_ids if doc_id not in doc_ids]
...
def run_command(args: argparse.Namespace()):
    ...
    def run_update_command():
        if args.prop_value:
            logger.info(
                f'Updating "{args.prop_name}" to "{args.new_prop_value}" in items that have "{args.prop_name}" '
                f'set to "{args.prop_value}" in "{input_path}".')
            file_name = create_file_name(
    ...
    def run_clone_command():
        logger.info(
            f'Cloning items that have "{args.prop_name}" set to "{args.prop_value}" from "{input_path}" '
            f'and updating the property value to "{args.new_prop_value}".')
        file_name = create_file_name(
    ...
    def run_test_command():
        ...
        logger.info('Checking against the schema.')
        validate_against_schema(input_path, args.schema_path)
        logger.info('OK')
        if root_key_name in ['docs', 'sources']:
