...
function validateArgs() {
    ...
    if (!(availableResourceTypes.includes(resourceType) && availableAwsEnvs.includes(awsEnv))) {
...
function mergeAndStringifyTags(tags) {
    ...
    if (resourceType === 'ecr') {
        return allTags.map(obj => Object.entries(obj).map(([key, value]) => `"${key}"="${value}"`).join(',')).join(' ');
    }
...
if (resourceType === 'secrets-manager') {
    for (const secret of awsObjects) {
        ...
        const awsConfigureTagsCmd = `aws secretsmanager tag-resource --secret-id ${secretId} --tags '${mergeAndStringifyTags(tags)}'`
    ...
    for (const repo of awsObjects) {
        ...
        const awsConfigureTagsCmd = `aws ecr tag-resource --resource-arn "${resourceArn}" --tags ${mergeAndStringifyTags(tags)}`
    ...
    for (const bucket of awsObjects) {
        ...
        const awsConfigureTagsCmd = `aws s3api put-bucket-tagging --bucket "${bucketName}" --tagging '{"TagSet": ${mergeAndStringifyTags(tags)}}'`
