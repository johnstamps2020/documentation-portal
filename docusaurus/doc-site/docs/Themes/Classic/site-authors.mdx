...
1. If you updated the version, run `yarn install` to update the `yarn.lock` file.

To add site authors:
1. Open `docusaurus.config.js`.
1. Within the `config` object set to `module.exports`, add a `customFields` object if one does not yet exist. If there already is a `customFields` object, proceed to the next step.
