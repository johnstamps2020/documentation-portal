...
kubectl exec ${POD_NAME} --namespace=${NAMESPACE} -- sh -c 'rm -rf /usr/src/app/static/sitemap && mkdir /usr/src/app/static/sitemap && cp -R /tmp/out/* /usr/src/app/static/sitemap/ && rm -rf /tmp/out'