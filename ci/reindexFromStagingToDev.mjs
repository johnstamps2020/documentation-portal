async function checkTaskStatus(taskId, elapsedTime = 0) {
    ...
    if (elapsedTime > 3600000) {
        throw new Error("The reindexing task has been running for more than an hour. Terminating the build.")
    }
...
const elasticsearchDevUrl = "https://docsearch-doctools.dev.ccs.guidewire.net:443"
const indexName = "gw-docs"
const reindexTaskResponse = await fetch(`${elasticsearchDevUrl}/_reindex?wait_for_completion=false`,
    {
        method: "POST",
        headers: {
...
if (reindexTaskError) {
    throw new Error(`Reindexing task not started. Error: ${reindexTaskError.reason}`)
}
if (reindexTaskId === "") {
