...
kubectl exec -it "$(kubectl get pods -l app=redirect-int-to-staging -o jsonpath='{.items[0].metadata.name}' -n doctools)" -n doctools \
 -- /bin/sh -c 'echo "server { listen 80; listen 443; server_name _; return 301 https://docs.staging.ccs.guidewire.net$request_uri; }" > /etc/nginx/conf.d/nginx.conf && nginx -s reload'
