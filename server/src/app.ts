...
import { getOidcLoginRouter } from './routes/authorization-code';
...
*/
  ...
  const oidcLoginRouter = await getOidcLoginRouter();
  ...
  app.use('/google6a1282aff702e827.html', (req, res) => {
    res
      .status(200)
      .send(`google-site-verification: google6a1282aff702e827.html`);
  });
  ...
  if (isDevMode) {
    app.use(
      express.static(join(__dirname, '../../html5/static/html5'), options)
    );
  } else {
