...
To apply the patch, change the buildType with id = 'stagingBillingCenter1011bc1011'
accordingly, and delete the patch script.
*/
changeBuildType(RelativeId("stagingBillingCenter1011bc1011")) {
    check(paused == false) {
    ...
    expectSteps {
        ...
        script {
            ...
            scriptContent = """
                ...
                aws s3 sync ./${'$'}RESOURCE_DIR/ReleaseNotes-files/bc/10.x/ s3://%env.S3_BUCKET_NAME%/%env.PUBLISH_PATH%/extras/ReleaseNotes-files/bc/10.x --delete
            """.trimIndent()
        }
        script {
            name = "Copy resources from git to S3"
            ...
            scriptContent = """
                ...
                echo "Copying files to S3"
                aws s3 sync ./${'$'}RESOURCE_DIR/ReleaseNotes-files/cm/10.x/ s3://%env.S3_BUCKET_NAME%/%env.PUBLISH_PATH%/extras/ReleaseNotes-files/cm/10.x --delete
            """.trimIndent()
        }
    }
    steps {
